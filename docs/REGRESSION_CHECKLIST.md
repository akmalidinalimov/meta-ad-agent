...
| 2026-06-01 | 180-day Meta sync and chat handoff integration | `POST /api/meta/sync` with 180 days; `python -m pytest backend -q`; `npm test -- --run`; `npm run lint`; `npm run build`; `npm run test:e2e` | PASS | Saved a 180-day Meta snapshot with 65 campaigns, 123 ad sets, 100 ads, 1,477 base insight rows, and LLM analysis enabled. Chat responses now expose specialist handoffs and quality scores through the API/dashboard path. |
| 2026-06-01 | New-campaign refresh readiness and richer rankings | `python -m pytest backend -q`; `npm test -- --run`; `npm run lint`; `npm run build`; `npm run test:e2e` | PASS | Added global dashboard refresh, in-place refresh after Meta sync, local-dev direct FastAPI data fetch to avoid Vite proxy resets, cached dashboard builds keyed by knowledge/alerts timestamps, and richer ranking KPIs for CPC, CPL, lead rate, and Telegram START rate. |
| 2026-06-01 | Stronger monitoring and New Campaign Watch | `python -m pytest backend -q`; `npm test -- --run`; `npm run lint`; `npm run build`; `npm run test:e2e` | PASS | Monitoring now flags lead-rate drops and spend/click/no-lead issues with why-it-matters explanations. Dashboard Alerts now shows current campaign decisions for synced/new campaigns. |
| 2026-06-01 | Orchestrator decisions, draft proposals, and operator priority queue | `python -m pytest backend -q`; `npm test -- --run`; `npm run lint`; `npm run build`; `npm run test:e2e` | PASS | Orchestrator now returns an agent decision trace for multi-specialist tasks, campaign proposals can be generated as review-only paused drafts, and the dashboard prioritizes monitoring, campaign-watch, tracking, approval, and insight items for operators. |
| 2026-06-01 | Proposal UI, scheduled monitoring scaffold, and Telegram manager replies | `python -m pytest backend -q`; `npm test -- --run`; `npm run lint`; `npm run build`; `npm run test:e2e` | PASS | Strategy tab now shows review-only draft proposal controls, monitoring has a 4-hour cron-ready alert-only runner with logs, and Telegram replies include agent/quality/involved-agent context plus direct attention summaries. |
