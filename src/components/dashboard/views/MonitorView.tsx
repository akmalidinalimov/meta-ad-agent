// The lean Monitor screen: live per-campaign KPIs + funnel rates (primary), with
// the synced account history kept behind a disclosure, and the Agent Office.
...
interface MonitorViewProps {
  // `metrics` is accepted for API symmetry with the previous Monitor (callers still
  // pass the account-history rows) but the view now derives everything it shows from
  // the pre-computed `trend`/`funnel`, so it is intentionally not destructured here.
    ...
    <section className="monitor-screen">
      ...
      {hasHistory && (
        <details className="monitor-center" open={chartsOpen}>
          <summary>Account history · last sync</summary>
